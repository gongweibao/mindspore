...
bool HcomAllReduceKernel::Launch(const std::vector<AddressPtr> &inputs, const std::vector<AddressPtr> & /*workspace*/,
  ...
  MS_LOG(INFO) << "HcclAllReduce hccl_count_: " << hccl_count_ << ", data_type: " << hccl_data_type_list_[0] << ", op_type: " << op_type_;
  MS_LOG(ERROR) << "[start]HcclAllReduce input addr: " << inputs[0]->addr << ", output addr: " << outputs[0]->addr << ", stream ptr: " << stream_ptr;
  ...
  MS_LOG(ERROR) << "[done]HcclAllReduce input addr: " << inputs[0]->addr << ", output addr: " << outputs[0]->addr << ", stream ptr: " << stream_ptr;
